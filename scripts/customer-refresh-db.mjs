...
import {customerInventoryStatements} from '../src/customer-feed-query.ts';
...
export async function scope(){let captured='';customerInventoryStatements({prepare(sql){return {bind(...values){if(!captured){let index=0;captured=sql.replace(/\?/g,()=>sqlValue(values[index++]));}return this;}}}},new URL('https://source/inventory'));return captured.slice(0,captured.indexOf(' SELECT id,title,set_name,retailer,language,price_mxn,availability,observed_at'));}
