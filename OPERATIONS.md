...
Bulk `codex_seed` campaigns remain visible in `/approvals` but are excluded from ordinary per-item Discord retry delivery. Complete campaign review in the protected workspace and use the aggregate visibility-only action; it verifies the displayed count, publishes atomically, leaves Catch enrollment unchanged, and creates one idempotent campaign event. Deploy Catch's schema-3 consumer before applying migration `0024_seed_campaign_publication.sql` and deploying Spawn. Do not turn a large seed batch into a Discord notification burst.
