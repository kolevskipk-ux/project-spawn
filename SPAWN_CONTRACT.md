...
## Baseline-readiness amendment — 2026-09-07

Approved for local implementation and isolated validation; production activation remains separate. Relevant products may be ingested and explicitly approved across card languages. Language is recorded evidence, not an English-only gate. Unknown language must be clearly disclosed and explicitly acknowledged during identity resolution. Product-language variants must not share pricing references by assumption. Spawn owns identity/approval/catalog data; Catch owns monitoring state and customer delivery. Catalog v2 carries multilingual targets; deploy the dual-version Catch reader before the Spawn v2 publisher. Preserve existing identities, baseline keys and decision history. Monthly catalog maintenance is a read-only review queue; it does not activate a schedule, archive targets, or reset state.
