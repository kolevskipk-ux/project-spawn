...
async function runScan(env: Env, triggerSource: "cron" | "manual" | "early_asin"): Promise<{ id: string; result: ScanResult }> {
  ...
  try {
    ...
    try {
      const rawResult = await callOpenAI(env,id,triggerSource==="early_asin"?"early_asin":"market");
      ...
      await env.SPAWN_DB.batch([
        yieldStatement(env,id,inventory.changes,inventory.discoveries,inventory.baseline,finished),
        ...
        searchReviewStatement(env,id,finished),
...
async function sharedState(request: Request, url: URL, env: Env): Promise<Response | null> {
  ...
  if (request.method === "GET" && url.pathname === "/internal/garfield/amazon-watchlist") {
    const [version,rows]=await Promise.all([
      ...
      env.SPAWN_DB.prepare("SELECT asin,canonical_product_id,product_name,product_url,watch_category,language,priority,lane,poll_interval_minutes,COALESCE(routing_key_v2,routing_key) routing_key,alert_on_initial_buyable,approved_by,approval_reason,approved_at,source,last_discovered_at,updated_at,automatic_baseline,EXISTS(SELECT 1 FROM inventory i JOIN inventory_admin_reviews admin ON admin.listing_key=i.listing_key WHERE i.canonical_url=amazon_watchlist.product_url AND admin.removed_at IS NOT NULL) inventory_removed FROM amazon_watchlist WHERE lifecycle_status='PUBLISHED' ORDER BY CASE lane WHEN 'priority' THEN 0 ELSE 1 END, CASE priority WHEN 'BOSS' THEN 0 WHEN 'HIGH' THEN 1 ELSE 2 END, asin").all()
    ]);
    return json({schema_version:AMAZON_CATALOG_SCHEMA_VERSION,catalog_version:version?.value??"0",published_at:version?.updated_at??null,watchlist:rows.results.map(({automatic_baseline,inventory_removed,...row})=>({...row,...(automatic_baseline?{automatic_baseline:1}:{}),...(inventory_removed?{inventory_removed:1}:{})}))});
  }
