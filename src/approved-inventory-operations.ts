...
export async function approvedInventoryOperations(request:Request,env:Env,operator:Operator):Promise<Response>{
 ...
 if(request.method==='POST'){
  ...
  if(action==='edit'&&(!title||title.length>300||!PRODUCT_LANGUAGES.includes(language)))return operationsError('Enter a title and supported language.',400,operator,env);
