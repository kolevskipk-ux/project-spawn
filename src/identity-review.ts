...
export async function resolveAmazonIdentity(env: Env, asin: string, input: IdentityReview, actor: string, now = new Date()) {
  ...
  if (!row || row.lifecycle_status !== 'DISCOVERED') return {ok:false as const,error:'identity_review_unavailable'};
  ...
  if (row.outcome !== 'REVIEW_REQUIRED' || row.access_outcome !== 'VALID_PAGE' || !Number.isFinite(age) || age < 0 || age > 36 * 3_600_000 ||
  ...
  const identity=`amazon-${asin.toLowerCase()}-${input.language}`;
  ...
  const results=await env.SPAWN_DB.batch([
    env.SPAWN_DB.prepare(`INSERT INTO amazon_verification_attempts
      ...
      WHERE w.asin=? AND w.lifecycle_status='DISCOVERED' AND w.evidence_revision=?`)
    ...
    env.SPAWN_DB.prepare(`UPDATE amazon_watchlist SET verification_attempt_id=(SELECT id FROM amazon_verification_attempts WHERE evidence_revision=?),
      ...
      WHERE asin=? AND lifecycle_status='DISCOVERED' AND evidence_revision=?`)
