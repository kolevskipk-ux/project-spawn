...
export async function handleCatchInventoryObservation(request:Request,env:Env):Promise<Response>{
  ...
  const statements=[
    env.SPAWN_DB.prepare(`INSERT OR IGNORE INTO catch_inventory_observations(observation_id,listing_key,asin,observed_state,price_mxn,seller,fulfilled_by,evidence_type,observed_at,received_at,schema_version,source_owner,transition_id,delivery_outcome,price_verification_status) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`).bind(observation.observation_id,listingKey,observation.asin,observation.observed_state,observation.price_mxn,observation.seller,observation.fulfilled_by,observation.evidence_type,observation.observed_at,now,observation.schema_version,observation.source_owner,observation.transition_id,observation.delivery_outcome,observation.price_verification_status),
  ...
  return Response.json({accepted:true,replayed:false,listing_key:listingKey,enrichment_queued:Boolean(available&&observation.transition_id),customer_event_created:false},{status:202});
}
