...
export async function customerFetch(request: Request, env: CustomerEnv): Promise<Response> {
  ...
  if(path==='/' && ['GET','HEAD'].includes(request.method)) return page(env,'Explore the inventory',`<section class="hero"><div class="eyebrow">Your inventory dashboard</div><h1>See the full tracked inventory.</h1><p class="muted">Create a free account to browse the listings we’re tracking, check observed availability, and filter by set or store.</p><a class="button" href="/app">Get dashboard access</a><p>No invitation needed. Sign in with a code sent to your email.</p><p class="muted">Browser access only. Downloads are unavailable during the pilot.</p><p class="notice">This staging pilot uses sample listings, not live offers.</p></section>`);
  if(!['/app','/app/account','/app/join'].includes(path)) return page(env,'Page not found','<h1>Page not found</h1><a href="/app">Return to inventory</a>',undefined,404);
  if(!['GET','HEAD'].includes(request.method) && !(path==='/app/join' && request.method==='POST')) return page(env,'Action unavailable','<h1>Action unavailable</h1>',undefined,405);
  ...
  if(member?.status==='REVOKED') return page(env,'Access unavailable','<h1>Access unavailable</h1><p>Your dashboard access has been disabled. Please contact the Garfield team.</p>',undefined,403);
  ...
  if(path==='/app/account') return page(env,'My account',`<section class="account panel"><h1>My account</h1><p>${escape(member.email)}</p><p class="identity">Customer ID: ${escape(member.id)}</p><p>Access: customer · read only</p><p>Downloads: unavailable</p><p class="muted">For an account removal request, contact the Garfield team.</p><p><a href="/app">Back to inventory</a></p><a href="/cdn-cgi/access/logout">Sign out</a></section>`,member);
