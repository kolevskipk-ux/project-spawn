...
const headers = {
  ...
  'content-security-policy':"default-src 'none'; style-src 'unsafe-inline'; script-src 'self'; form-action 'self' https://discord.com; frame-ancestors 'none'; base-uri 'none'",
...
export async function customerFetch(request: Request, env: CustomerEnv): Promise<Response> {
  ...
  if(path==='/inventory-filters.js'&&request.method==='GET')return new Response("document.addEventListener('DOMContentLoaded',()=>{const form=document.querySelector('form.filters');if(!form)return;form.addEventListener('change',event=>{if(event.target.tagName!=='SELECT')return;if(event.target.name==='tcg')form.elements.namedItem('set').value='';form.requestSubmit();});});",{headers:{'content-type':'text/javascript; charset=utf-8','cache-control':'public, max-age=300','x-content-type-options':'nosniff'}});
