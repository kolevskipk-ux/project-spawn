...
async function revalidateBatch(env:Env,now:Date,fetchFn:typeof fetch){
  ...
  const candidates=await env.SPAWN_DB.prepare(`SELECT i.listing_key,i.canonical_url,i.retailer,i.title,i.status,i.price_mxn,c.routing_key,s.due_at,s.next_eligible_at,i.fulfilment_region_state,i.retailer_country,i.ship_from_country,i.original_price,i.original_currency,i.mexico_delivery_status,i.shipping_mxn,i.import_cost_status,i.destination_checked_at,i.destination_fresh_until
    ...
    WHERE ${env.STORE_MONITORING_ENABLED==='true'?"NOT EXISTS(SELECT 1 FROM store_monitor_targets t JOIN store_acquisitions a ON a.id=t.store_id WHERE t.id=i.listing_key AND a.status='APPROVED' AND t.acknowledgement_at IS NOT NULL) AND ":''}COALESCE(s.lifecycle_state,'ACTIVE')!='ARCHIVED' AND COALESCE(s.next_eligible_at,i.last_seen_at)<=? AND NOT EXISTS(SELECT 1 FROM amazon_watchlist w WHERE w.product_url=i.canonical_url AND (w.lifecycle_status='PUBLISHED' OR w.staging_enabled=1))
