...
export async function operationsRoute(request: Request, env: Env, operator: Operator): Promise<Response | null> {
  ...
  if (path==='/ops') {
    ...
    title='My account';content=`<section><h2>${esc(operator.email)}</h2><p>Role: <strong>${esc(operator.role)}</strong></p><p>Your identity is verified by the website’s sign-in provider. Sessions are limited to eight hours; you may be asked to sign in sooner.</p><a class="ops-primary" href="/cdn-cgi/access/logout">Sign out</a></section>`;
