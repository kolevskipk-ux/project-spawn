...
export function operationsShell(title: string, content: string, operator: Operator, env: Env, path: string, styles = ''): string {
  const links = [['/ops','Overview'],['/approvals','Approvals'],['/inventory','Inventory'],['/ops/vendors','Vendors'],['/ops/health','System health'],['/ops/activity','Activity'],...(operator.role === 'owner' ? [['/ops/people','People & roles']] : []),...(env.CUSTOMER_DB && operator.role!=='viewer' ? [['/ops/customers','Customers']] : []),['/ops/account','My account']];
