...
const EVENT_TYPES=new Set(["LISTING_PUBLISHED","BECAME_BUYABLE","PRICE_DROP"]);
...
export async function handleCustomerEvents(request:Request,url:URL,env:Env):Promise<Response|null>{
  ...
  if(url.pathname.endsWith("/ack")){
    ...
    const current=await env.SPAWN_DB.prepare("SELECT delivery_status FROM customer_inventory_events WHERE event_id=?").bind(ack.event_id).first<{delivery_status:string}>();
    ...
    await env.SPAWN_DB.prepare("UPDATE customer_inventory_events SET delivery_status=?,acknowledged_at=? WHERE event_id=? AND delivery_status NOT IN ('DELIVERED','SUPPRESSED')").bind(ack.status,now,ack.event_id).run();
  ...
  const rows=await env.SPAWN_DB.prepare(`SELECT event_id,schema_version,event_type,listing_key,source_observation_id,routing_key,payload_json,occurred_at,created_at
