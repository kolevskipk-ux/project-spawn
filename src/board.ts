...
.filter-actions{grid-column:1/-1;display:flex;flex-wrap:wrap;gap:10px}.filter-actions button,.filter-actions .download{min-height:44px;padding:10px 18px;border-radius:10px;font:inherit;font-weight:800}.filter-actions button{border:1px solid var(--lime);background:var(--lime);color:#15180f;cursor:pointer}.filter-actions .download{background:transparent;border:1px solid var(--line);color:var(--text)}.filter-actions button:focus-visible,.filter-actions a:focus-visible{outline:3px solid var(--lime);outline-offset:3px}
...
<div class="filter-actions"><button id="apply-filters" type="button">Apply filters</button><a class="download" href="/inventory.csv?access=${encodeURIComponent(accessToken)}">Excel / CSV</a></div></section>
...
document.getElementById('apply-filters').addEventListener('click',filter);
controls.forEach(control=>control.addEventListener('change',filter));
document.getElementById('search').addEventListener('keydown',event=>{if(event.key==='Enter'){event.preventDefault();filter()}});
filter();
</script></body></html>`;
}
