...
export async function claimAmazonBrowserAdvisories(env:Env,now=new Date()){
 ...
 for(const item of ITEMS){
  ...
  const latest=await env.SPAWN_DB.prepare('SELECT id,observed_at,buying_options_shown FROM amazon_browser_observations WHERE asin=? ORDER BY observed_at DESC,received_at DESC,id DESC LIMIT 1').bind(item.asin).first<{id:string;observed_at:string;buying_options_shown:number|null}>();
  if(!latest||latest.observed_at<cutoff||latest.observed_at>time||latest.buying_options_shown===null)continue;
  const id=`amazon-options:${item.asin}:${latest.id}`;
  ...
  await env.SPAWN_DB.batch([
   ...
   env.SPAWN_DB.prepare(`INSERT OR IGNORE INTO amazon_browser_advisories(id,asin,observation_id,observed_at,created_at,expires_at)
    SELECT ?,?,?,?,?,? FROM amazon_browser_advisory_state WHERE asin=? AND last_observed_at<? AND last_signal=0 AND ?=1 AND (last_queued_at IS NULL OR last_queued_at<=?)`).bind(id,item.asin,latest.id,latest.observed_at,time,expires,item.asin,latest.observed_at,latest.buying_options_shown,cooldown),
   env.SPAWN_DB.prepare(`UPDATE amazon_browser_advisory_state SET last_signal=?,last_observed_at=?,last_queued_at=CASE WHEN EXISTS(SELECT 1 FROM amazon_browser_advisories WHERE id=?) THEN ? ELSE last_queued_at END WHERE asin=? AND last_observed_at<?`).bind(latest.buying_options_shown,latest.observed_at,id,time,item.asin,latest.observed_at),
   env.SPAWN_DB.prepare("UPDATE amazon_browser_advisories SET status='EXPIRED' WHERE asin=? AND status='PENDING' AND (expires_at<=? OR ?=0)").bind(item.asin,time,latest.buying_options_shown)
  ]);
  if(latest.buying_options_shown!==1)continue;
  const claimToken=crypto.randomUUID(),lease=new Date(now.getTime()+120000).toISOString();
  const claimed=await env.SPAWN_DB.prepare(`UPDATE amazon_browser_advisories SET claim_token=?,lease_until=?,attempts=attempts+1 WHERE id=(SELECT id FROM amazon_browser_advisories WHERE asin=? AND status='PENDING' AND expires_at>? AND (lease_until IS NULL OR lease_until<=?) ORDER BY created_at DESC LIMIT 1) AND status='PENDING' AND (lease_until IS NULL OR lease_until<=?) RETURNING id,asin,observed_at,expires_at`).bind(claimToken,lease,item.asin,time,time,time).first<{id:string;asin:string;observed_at:string;expires_at:string}>();
  if(claimed)events.push({...claimed,claim_token:claimToken,kind:'BUYING_OPTIONS_SHOWN',routing_key:'pokemon-30th',url:item.url,product_name:item.name,stock_verified:false});
 }
 return events;
}
