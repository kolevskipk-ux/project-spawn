...
export async function trustedStoreOperations(request:Request,env:Env,operator:Operator):Promise<Response>{
  if(request.method==='POST'){
    ...
    if(action==='revoke')await env.SPAWN_DB.prepare('UPDATE trusted_store_rules SET enabled=0,revoked_at=?,revoked_by=? WHERE id=? AND enabled=1').bind(now,operator.email,id).run();
